...
function FarmerHome() {
  ...
  useEffect(async () => {
    ...
    try{    
    const data = await FarmerService.get(parseJwt(localStorage.getItem('Farmer'), { decrypt: true}).iss)
      .then((response) => {
    ...
    const options = {
      ...
      handler:async function (response) {
        if (response.razorpay_payment_id !== null) {
          setLoading(true)
          const data =await ShipmentService.agree(id, st, response.razorpay_payment_id)
            .then(response => {
              setPay(false)
            })
            .catch(error => {
  ...
  const handleSubmitPayment = async (id, st, tot_pay) => {
    ...
    const options = {
      ...
      handler:async function (response) {
        if (response.razorpay_payment_id !== null) {
          setLoading(true)
          const data =await ShipmentService.agree(id, status, response.razorpay_payment_id)
            .then(response => {
              setPay(false)
            })
            .catch(error => {
                            ...
                            <div className="container-fluid">
                              <table class="table table-borderless">
                                <tbody>
                                  ...
                                  {order.consultFeePaid === "Paid" && order.paid === "Paid" && order.shipmentStatus !== "Refused" &&
                                    <tr>
                                    ...
                                    {order.shipmentStatus === "Refused"  &&
                                    <tr>
                                      <td></td>
                                      <td></td>
                                      <td> <b>You refused to buy this order. Thank you for using Pesticide Management System. Your total amount for pesticides Rs.{order.totalPay}/- will be refunded shortly.</b></td>
                                    </tr>
                                  }
                                </tbody>
                              </table>
