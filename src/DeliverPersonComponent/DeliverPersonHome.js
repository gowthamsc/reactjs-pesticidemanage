...
function DeliverPersonHome() {
  ...
  useEffect(async () => {
    ...
    try{
    const data = await DeliverPersonService.get(parseJwt(localStorage.getItem('DeliverPerson'), { decrypt: true}).iss)
      .then((response) => {
  ...
  const data=await  LoginService.logout(logoutDTO)
      .then((response) => {
        ...
        localStorage.removeItem("DeliverPerson");
        setLoading(false)
        navigate('/login')
      })
  };
    ...
    <>
      ...
      {!loading &&
        <>
          ...
          {isShipmentUpdate ?
          waitingCount===0? <h5><center>***No order waiting for you***</center></h5>:
          <>
