...
function ConsultantHome() {
  ...
  const [arrayLoad, setArrayLoad] = useState(true)
  const [loading, setLoading] = useState({ "initial": false, "array": arrayLoad });
  ...
  useEffect(async () => {
    ...
    setLoading({ "initial": true, "array": arrayLoad })
...
try{    
const data = await ConsultantService.get(parseJwt(localStorage.getItem('Consultant'), { decrypt: true}).iss)
      .then((response) => {
  ...
  const consultantCart =async () => {
    setArrayLoad(true)
    setLoading({ "initial": true, "array": arrayLoad })
    const data1 =await DiseaseService.getByConsultant(consultantEmail)
      .then((response) => {
      ...
      setArrayLoad(true)
    setLoading({ "initial": false, "array": arrayLoad })
  };
  ...
  const handleSubmitViewConsultant = async (event) => {
    ...
    setLoading({ "initial": true, "array": true })
  ...
  return (
    <>
      {loading && <Backdrop sx={{ color: '#fff', zIndex: (theme) => theme.zIndex.drawer + 1 }} open={loading.initial && loading.array} onClick={handleClose}>
        <CircularProgress color="inherit" />
      </Backdrop>}
      {!(loading.initial && loading.array) && <>
                                ...
                                <tr>
                                  ...
                                  {Object.keys(dropdown_Pest.pestName).length === 0 ? <></> : <>
                                    <td data-title="Required Qty">
                                      <input type="number" placeholder="Req Qty" name="req" id="reqQty" style={{ width: "100px" }}  min="0"/>
                                    </td>
                                    <td>
                                      <button type="button" className="btn btn-primary" onClick={
                                        () => {
                                          ...
                                          else if(document.getElementById("reqQty").value <0){
                                            setAddPest_error(true)
                                            setAddPest_errorMsg("Rquired quantity should not less than zero")
                                          }
                                          else {
