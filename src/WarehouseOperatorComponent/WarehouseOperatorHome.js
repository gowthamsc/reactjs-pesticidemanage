...
function WarehouseOperatorHome() {
  ...
  const [avlQtyInGm, setAvlQtyInGm] = useState();
  const [pricePerGm, setPricePerGm] = useState()
  ...
  useEffect(async () => {
    ...
    try{
    const data = await WarehouseOperatorService.get(parseJwt(localStorage.getItem('WarehouseOperator'), { decrypt: true}).iss)
      .then((response) => {
  ...
  const logout =async () => {
   ...
   const data =await LoginService.logout(logoutDTO)
      .then((response) => {
        ...
        localStorage.removeItem("WarehouseOperator");
        setLoading(false)
        navigate('/login')
      })
  };
