...
try{    
const data = await AdminService.get(parseJwt(localStorage.getItem('Admin'), { decrypt: true}).iss)
  ...
  const handleSubmitViewAdmin = async (event) => {
    event.preventDefault();
    setLoading(true)
  // console.log(JSON.stringify(updateAdmin))
   var email=adminEmail
    const data = await AdminService.update(email, updateAdmin)
      .then(response => {
        setChangeEffect(false)
      })
      .catch(error => {
